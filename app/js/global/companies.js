...
$.ajax({
    ...
    success: function(r) {
        ...
        $.each(r, function(i, value) {
            $select.append(`<option value = ${value.id_company}> ${value.company_name} </option>`);
        });
    },
});
        ...
        success: function(business) {
            ...
            for (let i = 0; i < business.length; i++) {
                if (company == business[i].company) {
                    $select.append(`<option value = ${business[i].id_business}>${business[i].name_business}</option>`);
                }
            }
        },
    });
