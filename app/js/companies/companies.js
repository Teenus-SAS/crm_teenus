$(document).ready(function () {
  ...
  $(document).on("click", ".editCompany", function (e) {
    ...
    let rowIndex = $(this).parent().parent()[0];
    let data = tableCompanies.fnGetData(rowIndex);

    //let row = $(this).closest("tr")[0];
    //let innerHTML = rowIndex.innerHTML;
    ...
    $(`#selectCategory option:contains(${data.category})`).prop(
      "selected",
      true
    );
  ...
  async function categories(category, subcategory) {
    try {
      ...
      $(`#selectSubcategory option:contains(${subcategory})`).prop(
        "selected",
        true
      );
    } catch (error) {}
